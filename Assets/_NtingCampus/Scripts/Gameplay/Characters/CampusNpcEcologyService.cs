...
namespace NtingCampus.Gameplay.Characters
{
    ...
    public sealed class CampusNpcEcologyService : MonoBehaviour
    {
        ...
        public bool TryBuildInteractiveLine(
            ...
            out string line)
        {
            ...
            string targetActorId = NormalizeActorId(actorId);
            bool hasRelationshipTarget = !string.IsNullOrEmpty(targetActorId);
            int suspicion = hasRelationshipTarget ? data.GetRelationshipSuspicion(targetActorId) : 0;
            int trust = hasRelationshipTarget ? data.GetRelationshipTrust(targetActorId) : 50;

            if (hasRelationshipTarget && suspicion >= 82)
            {
                if (data.Role == CampusCharacterRole.Teacher)
                {
                    line = CampusGameplayDebugTextCatalog.Get(
                        language,
                        "You have been turning up at convenient times.",
                        "You have been turning up at convenient times.");
                    return true;
                }
                ...
                if (data.Role == CampusCharacterRole.Staff)
                {
                    line = CampusGameplayDebugTextCatalog.Get(
                        language,
                        "Do not get too close to the counter.",
                        "Do not get too close to the counter.");
                    return true;
                }
                ...
                if (data.HasTrait(CampusCharacterTrait.Tattletale))
                {
                    line = CampusGameplayDebugTextCatalog.Get(
                        language,
                        "I already told people to watch you.",
                        "I already told people to watch you.");
                    return true;
                }
            }

            if (hasRelationshipTarget && trust <= 22)
            {
                line = CampusGameplayDebugTextCatalog.Get(
                    language,
                    "I do not want to be tied to you right now.",
                    "I do not want to be tied to you right now.");
                return true;
            }

            if (hasRelationshipTarget && trust >= 74 && data.HasTrait(CampusCharacterTrait.Troublemaker))
            {
                line = CampusGameplayDebugTextCatalog.Get(
                    language,
                    "That thing you pulled had nerve.",
                    "That thing you pulled had nerve.");
                return true;
            }
            ...
            if (data.Mood <= 20)
            {
                line = CampusGameplayDebugTextCatalog.Get(
                    language,
                    "Today is already annoying enough.",
                    "Today is already annoying enough.");
                return true;
            }

            if (data.SocialEnergy <= 16 && taskType != CampusCharacterTaskType.TeachClass)
            {
                line = CampusGameplayDebugTextCatalog.Get(
                    language,
        ...
        public bool TryBuildAmbientLine(
            CampusCharacterData data,
            CampusCharacterTaskType taskType,
            out string line)
        {
            ...
            if (gossipHeat >= 72 && data.Role == CampusCharacterRole.Teacher)
            {
                line = CampusGameplayDebugTextCatalog.Get(
                    language,
                    "There are too many rumors today.",
                    "There are too many rumors today.");
                return true;
            }

            if (ResolveHighestRelationshipSuspicion(data) >= 88 && data.Role != CampusCharacterRole.Student)
            {
                line = CampusGameplayDebugTextCatalog.Get(
                    language,
                    "This student needs an extra look.",
                    "This student needs an extra look.");
                return true;
            }

            if (data.Mood <= 18 && taskType == CampusCharacterTaskType.Socialize)
            {
                line = CampusGameplayDebugTextCatalog.Get(language, "Do not ask.", "Do not ask.");
                return true;
            }
