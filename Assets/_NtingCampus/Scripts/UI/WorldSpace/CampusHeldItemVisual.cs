...
namespace NtingCampus.Gameplay.Characters
{
    ...
    public sealed class CampusHeldItemVisual : MonoBehaviour
    {
        ...
        private int lastHandsStateHash;
        ...
        public void RefreshImmediate()
        {
            ...
            if (!visible)
            {
                lastHandsStateHash = 0;
                SetCanvasVisible(false);
                return;
            }

            StorageContainerModel[] hands = CampusHandInventoryUtility.ResolveHands(runtime);
            int stateHash = CampusHandInventoryUtility.BuildHandsStateHash(hands);
            if (stateHash == lastHandsStateHash && canvas != null)
            {
                return;
            }

            lastHandsStateHash = stateHash;
            StorageItemModel leftItem = CampusHandInventoryUtility.ResolveHeldItem(hands, 0);
            StorageItemModel rightItem = CampusHandInventoryUtility.ResolveHeldItem(hands, 1);
            bool hasAnyItem = leftItem != null || rightItem != null;
            SetCanvasVisible(hasAnyItem);
            if (!hasAnyItem)
            {
                ApplyItem(leftHandImage, leftHandLabel, null);
                ApplyItem(rightHandImage, rightHandLabel, null);
                return;
            }
        ...
        private static void ApplyItem(Image image, Text label, StorageItemModel item)
        {
            ...
            image.sprite = icon;
            image.color = icon != null ? Color.white : Color.Lerp(item.ThemeColor, Color.white, 0.12f);
            label.text = icon != null ? string.Empty : BuildShortLabel(item);
        }
