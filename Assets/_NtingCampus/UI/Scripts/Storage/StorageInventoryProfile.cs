...
namespace Nting.Storage
{
    ...
    public sealed class StorageContainerDefinition
    {
        ...
        public void ApplyRuntimeSettings(StorageContainerModel container, string ownerId, string roomId)
        {
            ...
            if (IsSingleItemSlot)
            {
                container.ApplyShape(Columns, Rows, true);
            }
            else
            {
                container.IsSingleItemSlot = false;
            }
    ...
    public sealed class StorageInventoryProfile : ScriptableObject
    {
        ...
        public static StorageInventoryProfile CreateDefaultPlayerProfile()
        {
            ...
            profile.Containers.Add(Carried("hand_left", StorageTextId.LeftHand, 1, 1, 3f, true));
            profile.Containers.Add(Carried("hand_right", StorageTextId.RightHand, 1, 1, 3f, true));
        ...
        private static StorageContainerDefinition Carried(
            string id,
            StorageTextId nameId,
            int columns,
            int rows,
            float maxWeight,
