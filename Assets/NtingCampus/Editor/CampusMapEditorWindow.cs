...
namespace NtingCampusMapEditor
{
    ...
    public sealed class CampusMapEditorWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/Nting Campus/Map Editor")]
        public static void Open()
        {
            CampusMapEditorWindow window = GetWindow<CampusMapEditorWindow>("Campus Map Editor");
        ...
        private void DrawLightingSection()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button(Text("Ensure Default Lighting", "生成默认光源")))
            {
                Light2D light = CampusMapEditorUtility.EnsureMapLighting(mapRoot);
                if (light != null)
                {
                    selectedLight = light;
                    Selection.activeGameObject = light.gameObject;
