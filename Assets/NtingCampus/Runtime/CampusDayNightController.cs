...
namespace NtingCampusMapEditor
{
    ...
    public sealed class CampusDayNightController : MonoBehaviour
    {
        ...
        [SerializeField] private bool externalTimeDriven;
        ...
        public bool ExternalTimeDriven => externalTimeDriven;
        ...
        private static void EnsureRuntimeController()
        {
            ...
            CampusDayNightController controller = EnsureSceneController(root);
            if (controller == null)
            {
                return;
            }
        ...
        public static CampusDayNightController EnsureSceneController(CampusMapRoot preferredRoot = null)
        {
            ...
            if (controller == null)
            {
                CampusMapRoot root = preferredRoot != null
                    ? preferredRoot
                    : Object.FindFirstObjectByType<CampusMapRoot>(FindObjectsInactive.Include);
        ...
        /// 使用外部玩法时钟设置当前日夜小时。
        /// </summary>
        public void ApplyExternalGameHour(float gameHour)
        {
            externalTimeDriven = true;
            Day01 = Mathf.Repeat(gameHour / HoursPerDay, 1f);
            hasInitializedTime = true;
            ApplyCurrentTimeState();
        }

        /// <summary>
        /// 解除外部玩法时钟驱动，恢复日夜系统自行推进。
        /// </summary>
        public void ReleaseExternalTimeDriver()
        {
            externalTimeDriven = false;
        }
        ...
        private void ResolveSceneReferences(bool force = false)
        {
            ...
            bool createMissingSceneObjects = mapRoot != null || !Application.isPlaying;
            if (sunLight == null && createMissingSceneObjects)
            {
            ...
            if (globalLight == null && createMissingSceneObjects)
            {
        ...
        private void UpdateSunPosition()
        {
            ...
            Vector2 offset2D = ResolveSunOrbitOffset(Day01, orbitRadius);
        ...
        private static Vector2 ResolveSunOrbitOffset(float day01, float radius)
        {
            float theta = Mathf.Repeat(day01, 1f) * Mathf.PI * 2f;
            return new Vector2(-Mathf.Sin(theta), -Mathf.Cos(theta)) * Mathf.Max(0f, radius);
        }
        ...
        private void UpdateSunLight(float intensity, Color color)
        {
            ...
            sunLight.lightType = Light2D.LightType.Point;
            sunLight.gameObject.name = CampusObjectNames.SunLight2D;
            sunLight.blendStyleIndex = 0;
