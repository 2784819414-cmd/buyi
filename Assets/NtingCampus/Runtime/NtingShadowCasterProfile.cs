...
namespace NtingCampusMapEditor
{
    ...
    public sealed class NtingShadowCasterProfile : MonoBehaviour
    {
        ...
        public static NtingShadowCasterProfile EnsureForPlacedObject(CampusPlacedObject placed)
        {
            ...
            NtingShadowCasterProfile profile = placed.GetComponent<NtingShadowCasterProfile>();
            if (profile == null)
            {
                profile = placed.gameObject.AddComponent<NtingShadowCasterProfile>();
        ...
        private bool TryResolveSpriteFrame(out SpriteShadowFrame frame)
        {
            ...
            float sourceWidth = Mathf.Max(MinSpriteWorldSize, Mathf.Abs(localSpriteSize.x * sourceScale.x));
            float sourceHeight = Mathf.Max(MinSpriteWorldSize, Mathf.Abs(localSpriteSize.y * sourceScale.y));
            Vector3 sourceCenter = sourceTransform.TransformPoint(sprite.bounds.center) + sourceTransform.TransformVector(new Vector3(localOffset.x, localOffset.y, 0f));
        ...
        private void ConfigureProxy(
            ...
            int sortingOrder)
        {
            if (proxy == null || sourceSpriteRenderer == null || sourceSpriteRenderer.sprite == null)
            {
                return;
            }
            ...
            float sourceHeight = Mathf.Max(MinSpriteWorldSize, Mathf.Abs(localSpriteSize.y * sourceScale.y));
            Vector3 sourceCenter = sourceTransform.TransformPoint(sprite.bounds.center) + sourceTransform.TransformVector(new Vector3(localOffset.x, localOffset.y, 0f));
            Vector3 sourceRight3 = sourceTransform.TransformDirection(Vector3.right).normalized;
            Vector3 sourceUp3 = sourceTransform.TransformDirection(Vector3.up).normalized;
            Vector2 sourceRight2 = new Vector2(sourceRight3.x, sourceRight3.y);
            Vector2 sourceUp2 = new Vector2(sourceUp3.x, sourceUp3.y);
            Vector2 normalizedDirection = direction.sqrMagnitude > 0.0001f ? direction.normalized : Vector2.down;
            Vector2 localShadowDelta = new Vector2(Vector2.Dot(normalizedDirection, sourceRight2), Vector2.Dot(normalizedDirection, sourceUp2)) * length;

            float minX = -sourceWidth * 0.5f + Mathf.Min(0f, localShadowDelta.x);
            float maxX = sourceWidth * 0.5f + Mathf.Max(0f, localShadowDelta.x);
            float minY = -sourceHeight * 0.5f + Mathf.Min(0f, localShadowDelta.y);
            float maxY = sourceHeight * 0.5f + Mathf.Max(0f, localShadowDelta.y);

            proxy.Transform.position = sourceCenter;
            proxy.Transform.rotation = Quaternion.identity;
            proxy.Transform.localScale = Vector3.one;
            UpdateProxyMesh(proxy, sourceRight3, sourceUp3, minX, maxX, minY, maxY);
            ...
            block.SetVector(SpriteUvMinSizeId, ResolveSpriteUvMinSize(sprite));
            block.SetVector(SourceCenterSizeId, new Vector4(sourceCenter.x, sourceCenter.y, sourceWidth, sourceHeight));
            block.SetVector(SourceRightId, new Vector4(sourceRight2.x, sourceRight2.y, 0f, 0f));
            block.SetVector(SourceUpId, new Vector4(sourceUp2.x, sourceUp2.y, 0f, 0f));
            block.SetVector(ShadowDirId, new Vector4(normalizedDirection.x, normalizedDirection.y, 0f, 0f));
            block.SetColor(ShadowColorId, finalColor);
            block.SetFloat(ShadowLengthId, Mathf.Max(0f, length));
            block.SetFloat(ShadowAlphaId, Mathf.Clamp01(alpha));
